...
public class StickyScrollLinearLayout extends LinearLayout implements NestedScrollingParent {
    ...
    public void setInitInterface(@NonNull StickyScrollInitInterface initInterface) {
        if (initInterface == null) throw new NullPointerException("initInterface can not be null!");
        this.mTopView = initInterface.setTopView();
        if (this.mTopView != null) getTopViewHeight();

        this.mTabView = initInterface.setTabView();

        this.mContentView = initInterface.setContentView();
        if (this.mContentView == null) return;
        setTotalHeight();
        requestLayout();
    }
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        Log.e(TAG, "mTopViewHeight == " + mTopViewHeight + "\ndy == " + dy + "\nscrollY == " + scrollY + "\nhiddenTop && showTop " + temp);
        if (!temp) {
            // judge
            temp = dy < 0 && (scrollY >= 0) && (!ViewCompat.canScrollVertically(target, -1) || firstVisiblePosition == targetFirstVisiblePosition);
...
//        mContentView = findViewById(R.id.vp);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mTabView == null || mTopView == null || mContentView == null) return;
//        getChildAt(0).
//                measure(
//                        widthMeasureSpec,
...
//                );
    ...
    private void setTotalHeight() {
        ...
        setMeasuredDimension(getMeasuredWidth(), mTopView.getMeasuredHeight() + mTabView.getMeasuredHeight() + mContentView.getMeasuredHeight());
    }
